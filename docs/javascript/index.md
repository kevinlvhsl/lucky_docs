
# 前端学习

## [技术路线图](./learn.md)

## 一、JavaScript 基础知识

* [1. 读书笔记：你不知道的Javascript上](./base/你不知道的Javascript上.md)
* [2. 读书笔记：你不知道的Javascript中](./base/你不知道的Javascript中.md)
* [3. 读书笔记：你不知道的Javascript下](./base/你不知道的Javascript下.md)
* [4. new A()和A()的区别](./base/new_A和A的区别.md)
* [5. Javascript的变量提升和函数提升](./base/Javascript的变量提升和函数提升.md)
* [6. Javascript对闭包的理解](./base/Javascript对闭包的理解.md)
* [7. 对Object.create和类式继承的理解](./base/对Object.create和类式继承的理解.md)
* [8. Object.create()和New的区别](./base/Object.create和New的区别.md)
* [9. Javascript柯里化和偏函数实现](./base/Javascript柯里化和偏函数实现.md)
* [10. Javascript中this绑定详解](./base/Javascript中this绑定详解.md)
* [11. JavaScript中改变this指向有几种方式](./base/JavaScript中改变this指向有几种方式.md)
* [12. Javascript的call和apply和bind方法](./base/Javascript的call和apply和bind方法.md)
* [13. Javascript的bind的polyfill的理解](./base/Javascript的bind的polyfill的理解.md)
* [14. Javascript模块化发展AMD,CMD,CommonJS,UMD,ES6](./base/Javascript模块化发展AMD,CMD,CommonJS,UMD,ES6.md)
* [15. Javascript寄生式组合继承详解](./base/Javascript寄生式组合继承详解.md)
* [16. JavaScript常见ployfill](./base/JavaScript常见ployfill.md)


## 二、JavaScript 进阶

* [1. JavaScript 算法与数据结构](https://github.com/trekhleb/javascript-algorithms/blob/master/README.zh-CN.md)
* [2. Immutable.js了解一下](https://juejin.im/post/5ac437436fb9a028c97a437c)


## 三、前端打包及项目管理
* [1. lerna项目管理](./lerna.md)


## 四、前端性能优化

* [1. Vue代码层面性能优化](./performance/性能优化实践.md#一、代码层面性能优化)
* [2. webpack-优化构建速度](./performance/性能优化实践.md#二、webpack-优化构建速度)
* [3. 优化输出质量-压缩文件体积](./performance/性能优化实践.md#三、优化输出质量-压缩文件体积)
* [4. 优化输出质量-加速网络请求](./performance/性能优化实践.md#四、优化输出质量-加速网络请求)
* [5. 优化结果分析](./performance/性能优化实践.md#五、优化结果分析)





